...
ESLint aplica reglas de corrección a todo `src/`. En los módulos ya saneados (`src/application`, reglas puras del motor, scripts y E2E), `no-unused-vars` también es bloqueante. El legacy restante se endurece de forma incremental para evitar cambios cosméticos masivos.
